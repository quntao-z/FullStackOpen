...
const CountryInformation = ({country}) => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      countryService.getCountryWeather(country)])
    .then(([countryInfoData, countryWeatherData]) => {
      ...
      setCountryData(
        <div>
          ...
          <div>temperature: {temperatureC}°C / {temperatureF}°F</div>
          <img src={conditionImage} />
          <div>wind {windKPH} kph / {windMPH} mph</div>
        </div>
      );
